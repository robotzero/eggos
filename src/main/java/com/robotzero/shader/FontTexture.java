...
public class FontTexture extends Asset {
  ...
  public void loadAsset() throws Exception {
    fileName = font.getName().toLowerCase();
    ...
    try ( ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      assetData = ByteBuffer.allocateDirect(data.length);
      assetData.put(data, 0, data.length);
      assetData.flip();
    }

    texture = Texture.createTexture(width, height, assetData);
  }
